...
class SetOfLines:
    ...
    def get_all_intersection_points(self):
        ...
        spans_repeat_each_point = np.repeat(spans, size, axis=0) #that is a repeat of the spans, each span[i] is being repeated size times
        identity = np.identity(dim)
        identity_repeat_rows_all = np.repeat(identity.reshape(1, -1), size, axis=0).reshape(-1, dim)
        I_final = np.repeat(identity_repeat_rows_all, size, axis=0).reshape(size * dim, size * dim) #the final is an identity matrix that is duplicated in rows and cols in factor of size
        G_G_T_all_permutations = np.outer(spans, spans)  # this is a 2 dimensional matrix of blocks, where the (i,j)-th block is spans[i]*spans[j]^T
        I_minus_G_G_T_all_permutations = I_final - G_G_T_all_permutations
        I_minus_G_G_tag_blocks = self.blockshaped(I_minus_G_G_T_all_permutations, dim, dim) #it will take the big matrix that is built from many clocks and returns a stack of blocks matrices
        I_minus_G_G_T_s = I_minus_G_G_tag_blocks[0:len(I_minus_G_G_tag_blocks):size + 1]  # this is a 1 dimensional matrix of blocks, where the i-th block is spans[i]*spans[i]^T
        I_minus_G_G_T_s_concatenated = I_minus_G_G_T_s.reshape(1, -1).T.reshape(-1, dim).T #that is a 1 dimensional block matrix, where the i-th element is the matrix I-G_iG_i^T
        I_minus_G_G_T_s_F = np.dot(spans, I_minus_G_G_T_s_concatenated)
        I_minus_G_G_T_s_F = I_minus_G_G_T_s_F.reshape(-1, 1).reshape(-1, dim) # in this matrix, the i-th index is the dot product of spans[j] and the k-th (I-GG^T), for j=i/size and k=i%size
        I_minus_G_G_T_s_F_inv = np.linalg.pinv(I_minus_G_G_T_s_F.reshape(size ** 2, dim, 1))  # this matrix dimension is $size^2 \times dim$, where the i-th element is the point on the line j=i/size that are closest to the line m=i%size. that means: I_minus_G_G_T_s_F_inv[1] = ((I-G_1G_1^T)F_1)^+, I_minus_G_G_T_s_F_inv[2] = ((I-G_1G_1^T)F_2)^+,...,I_minus_G_G_T_s_F_inv[i] = ((I-G_jG_j^T)F_m)^+,
        ...
        I_minus_G_G_T_s_dot_f_minus_g = I_minus_G_G_T_s_dot_f_minus_g.reshape(-1, 1).reshape(size, -1, dim) # this matrix contains more than it needs to contain. The i-th element is (I-G_iG_i^T)(f_k-g_l), and we do not need the cases where i!=l, that is why we take the right subset in the folowing two rows
        inner_steps = np.arange(0, size ** 2, size + 1)
        I_minus_G_G_T_s_dot_f_minus_g_s = I_minus_G_G_T_s_dot_f_minus_g[:, inner_steps]
        I_minus_G_G_T_s_dot_f_minus_g_s = I_minus_G_G_T_s_dot_f_minus_g_s.reshape(-1, dim) # this matrix is the right matrix, where the i-th element is (I-G_iG_i^T)(f_j-g_i)
        final = np.multiply(I_minus_G_G_T_s_F_inv, I_minus_G_G_T_s_dot_f_minus_g_s) #each row in this matrix is ((I-G_iG_i^T)F_j)^{+}(I-G_iG_i^T)(f_j-g_i)
    ...
    def get_4_approx_points(self, k):
        ...
        intersection_points_repeat_each_row = np.repeat(intersection_points, size, axis=0).reshape(-1, dim) #each one of the points is repeated for size times, in order to calculate the distance from each point to the n lines
        displacements_repeat_all = np.repeat(displacements.reshape(1, -1), intersection_points_size, axis=0).reshape(-1, dim) #each displacement repeated for intersection_points_size_size times to match the number of points
        spans_repeat_all = np.repeat(spans.reshape(1, -1), intersection_points_size, axis=0).reshape(-1, dim) #the same for the spans
        ...
        intersection_points_size_minus_displacements_squared_norms = np.sum(np.multiply(intersection_points_repeat_each_row_minus_displacements_repeat_all,intersection_points_repeat_each_row_minus_displacements_repeat_all),axis=1)
        np.sum(np.multiply(intersection_points_repeat_each_row_minus_displacements_repeat_all, spans_repeat_all) ** 2, axis=1)
        intersection_points_minus_displacements_dot_spans = np.multiply(intersection_points_repeat_each_row_minus_displacements_repeat_all, spans_repeat_all) ** 2
        intersection_points_minus_displacements_dot_spans_squared_norms = np.sum(intersection_points_minus_displacements_dot_spans,axis=1)
        ...
        all_distances = all_distances.reshape(intersection_points_size,size) #the i,j-th element is the distance between the i-th point in intersection_points_size and the j-th line in the set
        all_k_combinations_of_all_distances = all_distances[all_k_combinations_of_all_incdices]  #this is an array of size_of_intersection_points_size*size elements, where the i,j-element is the distance between the i-th point in intersection_points_size to the j-th line in the set
        all_k_combinations_of_all_distances_reshaped = all_k_combinations_of_all_distances.reshape(-1,k,size) #reshaped for later calculation
        distances_for_each_combination_of_k_points = np.min(all_k_combinations_of_all_distances_reshaped[:,:], axis=1) #the i,j-th item is the distance from the i-th k-tuple from all points to the i-th line in the set
        sum_of_distances_from_k_tuples_to_lines = np.sum(distances_for_each_combination_of_k_points, axis=1) #the i-th element in this array is the sum of squared distanes between the i-th point in all_k_combinations_of_intersection_points_size_repeat_each_row to the lines in the set
        ...
        final_min_indices = all_k_combinations_of_all_incdices_reshaped[sum_of_distances_from_k_tuples_to_lines_min_index]
    ...
    def get_centers_for_given_clusters(self, current_indices_cluster):
        ...
        assert size == len(current_indices_cluster), "current_indices_cluster size is not the number of lines in the set"
        ...
        for i in range(k):
            indices_clustered_to_i = np.asarray(np.where(current_indices_cluster == i))[0] #all the indices that contains i in current_indices_cluster
            displacements_clustered_to_i = displacements[indices_clustered_to_i] #all the displacements in the i-th cluster
            cluster_i_center = np.mean(displacements_clustered_to_i, axis=0).reshape(-1,dim) #the center of cluster of lines, that minimizes the sum of squared distances to the lines in the cluster is the mean of the displacements in the cluster, under the assumption that each line is spanned by a unit vector and its displacements is the closest point in the line to the origin
            if i == 0:
    ...
    def get_sum_of_distances_to_centers(self, centers):
        ...
        centers_points_repeat_each_row = np.repeat(centers_points, self_size, axis=0).reshape(-1, dim) #this is a k*size array where every k points were duplicated size times
        self_displacements_repeat_all = np.repeat(self_displacements.reshape(1,-1), centers_size, axis=0).reshape(-1,dim) #this is a size*k array where every size displacements were duplicated k times
        self_spans_repeat_all = np.repeat(self_spans.reshape(1,-1), centers_size, axis=0).reshape(-1,dim) #this is a size*k array where every size spans were duplicated k times
        self_weights_repeat_all = np.repeat(self_weights.reshape(1,-1), centers_size, axis=0) #this is a size*k array where every size spans were duplicated k times
    ...
    def get_closest_lines_to_centers(self, centers, m, type):
        ...
        centers_by_cluster_indices = centers.get_points_from_indices(cluster_indices) #that is an array of size points, where the i-th element is the centers[cluster_indices[i]]
        ...
        centers_by_cluster_indices_minus_displacements_squared_norms = np.sum(np.multiply(centers_by_cluster_indices_minus_displacements,centers_by_cluster_indices_minus_displacements),axis=1)
        centers_mul_spans_squared_norms = np.sum(np.multiply(centers_by_cluster_indices_minus_displacements, self_spans) ** 2, axis=1)
